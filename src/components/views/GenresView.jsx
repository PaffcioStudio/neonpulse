...
export default function GenresView({ library, currentSong, isPlaying, compact, onPlay, onFavorite, onContextMenu }) {
  ...
  const genres = useMemo(() => {
    const map = {};
    library.forEach(s => {
      const g = (s.genre || t('otherGenre', { ns: 'library' })).trim();
      if (!map[g]) map[g] = [];
      map[g].push(s);
    });
    ...
    return (
      <div className="p-6">
        <button onClick={() => setSelectedGenre(null)} className="flex items-center gap-2 text-zinc-500 hover:text-white transition-colors mb-5 text-sm">
          <ArrowLeft size={16} /> {t('allGenres', { ns: 'library' })}
        </button>
        <div className="flex items-center gap-4 mb-6 flex-wrap">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-xs text-zinc-500 uppercase tracking-widest mb-0.5">{t('genreLabel', { ns: 'library' })}</p>
  ...
  if (genres.length === 0) {
    return (
      <div className="p-6 text-center text-zinc-600 py-24">
        <Tag size={48} className="mx-auto mb-3 opacity-20" /><p>{t('noGenresInLibrary', { ns: 'library' })}</p>
      </div>
    );
  }
  ...
  return (
    <div className="p-6">
      <h2 className="text-3xl font-black uppercase tracking-tight mb-1">{t('genres', { ns: 'common' })}</h2>
      <p className="text-xs text-zinc-500 mb-6">{t('genreCount', { ns: 'common', count: genres.length })}</p>
          ...
          return (
            <div key={name} onClick={() => setSelectedGenre([name, songs])}
              ...
              <div className="absolute bottom-0 left-0 right-0 p-3">
                ...
                <p className="text-[11px] text-white/60 mt-0.5">{pluralTracks(songs.length, t)}</p>
              </div>

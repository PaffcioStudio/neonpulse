...
const FIELDS = [
  { key: 'artist', label: 'Artysta',  icon: Users,   placeholder: 'Ustaw artystę dla wszystkich…' },
  { key: 'album',  label: 'Album',    icon: Disc3,   placeholder: 'Ustaw album dla wszystkich…' },
  { key: 'genre',  label: 'Gatunek',  icon: Radio,   placeholder: 'Ustaw gatunek dla wszystkich…' },
  { key: 'year',   label: 'Rok',      icon: Calendar, placeholder: 'Rok wydania…', type: 'number' },
];
    ...
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4"
      ...
      <div className="relative w-full max-w-md bg-zinc-900 border border-zinc-700/60 rounded-2xl shadow-2xl overflow-hidden">
        ...
        <div className="flex items-center gap-3 px-5 py-4 border-b border-zinc-800">
          ...
          <div>
            <span className="font-semibold text-white text-sm">Edytor tagów (wiele)</span>
            <p className="text-[11px] text-zinc-500 mt-0.5">
              {songs.length} {songs.length === 1 ? 'utwór' : songs.length < 5 ? 'utwory' : 'utworów'}
            </p>
          </div>
        ...
        <div className="px-5 pt-3 pb-2">
          <p className="text-[11px] text-zinc-600 mb-2 uppercase tracking-wide">Wybrane utwory</p>
        ...
        <div className="px-5 py-3 space-y-3 border-t border-zinc-800/60">
          <p className="text-[11px] text-zinc-500">Zaznacz pola które chcesz nadpisać:</p>
        ...
        {status && (
          <div className={`mx-5 mb-3 px-3 py-2 rounded-xl text-xs flex items-center gap-2 ${
            ...
            {status === 'success'
              ? <><CheckCircle2 size={13} /> Zapisano {songs.length} utworów</>
        ...
        <div className="px-5 pb-5 flex justify-end gap-2 border-t border-zinc-800/60 pt-3">
          ...
          <button onClick={handleSave} disabled={!anyEnabled || saving}
            ...
            {saving
              ...
              : <><Save size={13} />Zastosuj do {songs.length} tr.</>}
          </button>
        </div>
      </div>
    </div>
